...
const ProductCard = ({ card_title, title, card_description, price, imgSrc , iter}) => {
        ...
        <div className='border-[2px] border-slate-100 flex flex-col items-center p-2 rounded-xl hover:border-yellow-400 cursor-pointer'>
            <div onClick={() => nav(`/product/${card_title}/${title}`)} className='flex flex-col items-center'>
                <div className='relative'>
                    <img className='2xl:w-86 w-72 h-56 2xl:h-60 rounded-xl' src={imgSrc} />
                ...
                <div className='w-full'>
                    <p className='text-xs line-clamp-2 mt-1 w-64 text-center px-2 h-8'>{card_description}</p>
                </div>
                <hr className={`prominent-hr w-full my-3`} />
                <span className='text-md mt-4 text-red-600 font-semibold'>Rs. {price}</span>
            </div>
            <button onClick={handleCart} className={`px-4 py-2 bg-[rgb(210,0,0)] text-white transition-colors font-semibold duration-300 rounded-3xl md:text-[0.95rem] hover:bg-yellow-500 hover:text-black m-1 cursor-pointer`} title='Add to Cart'>Add to Cart</button>
        </div>
    )
}
